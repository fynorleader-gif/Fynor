...
# =====================================================================
BOT_TOKEN = "8571348311:AAHdIDPtl_jh7DWu_yzGcnuZDzHfDAg9Y3c"
HERO_SMS_API_KEY = "A3dAc9766A04A98bf320fA7bA99f6994"
HERO_SMS_URL = "https://hero-sms.com/stubs/handler_api.php"

# 🌡️ SERVER 2 (SMSOTPS) CREDENTIALS
SMS_OTPS_API_KEY = "aWAYxeDjXSzZ7r7qXSSCtq5vhY1wbC7sky0dU4qVrb53BPV5ViDfPCqeq5eE"
SMS_OTPS_HEADERS = {
    "Accept": "application/json",
    "Content-Type": "application/json",
    "X-API-KEY": SMS_OTPS_API_KEY
}
...
def send_force_join_msg(chat_id):
    text = "💪🏻 𝐖e𝐥𝐜𝐨𝐦e 𝐭𝐨 𝐙𝐲𝐫𝐨𝐒𝐌𝐒\n\n━━━━━━━━━━━━━━━━━━━━━━\n\n⚠️ *Aapne hamara update channel join nahi kiya hai!*\n\nBot use karne k liye pehle niche click karke join karein aur Verify par click karein.\n\n💬 Telegram = @ZyroSMS\n\n━━━━━━━━━━━━━━━━━━━━━━"
...
def gift_collect_amount(message, target_uid, friend_uname):
    ...
    receiver_receipt = (
        ...
        f"👤 𝐅𝐫𝐨𝐦\n➜ @{sender_uname}\n\n"
        f"💵 𝐀𝐦𝐨𝐮𝐧𝐭\n➜ ${amount:.2f}\n\n"
        "💙 𝐓𝐡e 𝐆𝐢𝐟𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐀𝐝𝐝𝐞𝐝 𝐭𝐨 𝐲𝐨𝐮𝐫 𝐁𝐚𝐥𝐚𝐧𝐜𝐞 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲.\n"
        "━━━━━━━━━━━━━━━━━━━━━━"
    )
...
def run_s1(call):
    ...
    if not is_admin(user_id, call.from_user.username):
        ...
        if balance < cost:
            bot.send_message(user_id, f"♎  𝐋𝐨𝐰 𝐰𝐚𝐥λ𝐞𝐭 𝐛𝐚ln𝐜𝐞! 𝐏𝐫𝐢𝐜𝐞 𝐫e𝐪𝐮𝐢𝐫e𝐝: ${cost:.2f}")
            return
...
# =====================================================================
# 🌡️ RUN SERVER 02 ENGINE (SMSOTPS AUTOMATED WITH PROVIDER FALLBACK)
# =====================================================================
...
def run_s2(call):
    ...
    try: 
        ...
        logger.error(f"Callback split mismatch S2: {e}")
        return
    ...
    # 🌟 PROVIDERS LIST
    providers_list = ["provider_a", "provider_b", "provider_c", "provider_d"]
    number_found = False
    ...
    for provider in providers_list:
        ...
        try:
            logger.info(f"Checking Server 2 stock via: {provider} for service {app_code}")
            response = session.post("https://api.smsotps.com/api/order-number", json=payload, headers=SMS_OTPS_HEADERS, timeout=12)
            res_json = response.json()
            
            if "id" in res_json and "phone" in res_json:
                activation_id = str(res_json["id"])
                
                # 🛑 PRICE CHECK CRITERIA (Max 0.05 USDT Allowed)
                api_price = float(res_json.get("price", res_json.get("cost", 0)))
                if api_price > 0.05:
                    logger.info(f"Skipping {provider}: Price (${api_price}) is higher than 0.05 USDT limit. Auto-canceling...")
                    try:
                        session.post(f"https://api.smsotps.com/api/cancel-number/{activation_id}", headers=SMS_OTPS_HEADERS, timeout=5)
                    except Exception:
                        pass
                    continue  # Filter trigger: Agle provider par jao
                
                phone_number = str(res_json["phone"]).lstrip('+')
                ...
                text = f"🌡️ 𝐒e𝐫𝐯𝐞𝐫 𝟐 ({provider.replace('_', ' ').upper()}) \n\nService = {app_name}\n━━━━━━━━━━━━━━━━━━━━━━\n🧾 𝐍𝐮𝐦𝐛𝐞𝐫 \n\n`+{phone_number}`\n\n━━━━━━━━━━━━━━━━━━━━━━\n⏳ 𝐖𝐚𝐢𝐭𝐢𝐧𝐠 𝐟𝐨𝐫 𝐎𝐓𝐏..."
                markup = InlineKeyboardMarkup(row_width=1)
                markup.add(
                    InlineKeyboardButton("📋 Copy Number", copy_text=CopyTextButton(text=f"+{phone_number}")), 
                    InlineKeyboardButton("❌ Cancel Number", callback_data=f"cancel_s2_{activation_id}")
                )
                bot.edit_message_text(text, user_id, call.message.message_id, reply_markup=markup)
                threading.Thread(target=poll_s2, args=(user_id, call.message.message_id, activation_id, phone_number, app_name, cost)).start()
                
                number_found = True
                break  # Successful match under 0.05 found, exit provider loop
        except Exception as e:
            logger.error(f"Server 2 loop exception for {provider}: {e}")
            continue

    if not number_found:
        bot.send_message(user_id, error_text)

def poll_s2(user_id, message_id, activation_id, phone_number, app_name, cost):
    start_time = time.time()
    while time.time() - start_time < 1200:
        time.sleep(5)
        if activation_id not in active_orders: return
        try:
            res = session.get(f"https://api.smsotps.com/api/number-status/{activation_id}", headers=SMS_OTPS_HEADERS, timeout=10)
            res_json = res.json()
            
            status = res_json.get("status")
            if status == "completed" or "sms_code" in res_json:
                otp_code = res_json.get("sms_code")
                ...
                markup.add(
                    InlineKeyboardButton("📋 Copy OTP", copy_text=CopyTextButton(text=otp_code)), 
                    InlineKeyboardButton("🔄 Get Again OTP", callback_data=f"again_s2_{activation_id}")
                )
                bot.send_message(user_id, text, reply_markup=markup)
                ...
                if activation_id in active_orders: del active_orders[activation_id]
                return
            elif status in ["canceled", "cancelled", "expired", "refunded"]:
                if activation_id in active_orders: del active_orders[activation_id]
                bot.send_message(user_id, "❌ Order session terminated.")
                return
        except Exception as e:
            logger.error(f"Error checking Server 2 status thread loop: {e}")
...
def cancel_s2_(call):
    ...
    try:
        session.post(f"https://api.smsotps.com/api/cancel-number/{act_id}", headers=SMS_OTPS_HEADERS, timeout=10)
    except Exception as e:
        logger.error(f"Error processing Server 2 cancellation API call: {e}")
...
def again_s2(call):
    ...
    act_id = call.data.split("_")[2]
    try:
        session.post(f"https://api.smsotps.com/api/resend-sms/{act_id}", headers=SMS_OTPS_HEADERS, timeout=10)
    except Exception as e:
        logger.error(f"Error handling Server 2 resend action trigger: {e}")
    bot.send_message(call.message.chat.id, "🔄 Fetching another dynamic OTP sequence context...")
...
def pay_gate_init(call):
    ...
    if gateway == "easypaisa":
        text = "<b>𝐀𝐜𝐜𝐨𝐮𝐧𝐭 𝐃𝐞𝐭𝐚𝐢𝐥𝐬 </b> 📃 \n\n<b>𝗔𝗰𝗰𝗼𝘂𝗻𝘁 :</b> Easypaisa\n<b><b><b><b>𝗡𝗮𝗺𝗲 :</b></b></b></b> Saira Bibi\n<b><b><b><b><b><b><b>𝗡𝘂𝗺𝗯𝗲𝗿 :</b></b></b></b></b></b></b> <code>03087886967</code>\n\n⚠️ <i><b><b>𝐏λ𝐞𝐚𝐬𝐞 𝐭𝐚𝐤𝐞 𝐚 𝐒𝐜𝐫𝐞𝐞𝐧𝐬𝐡𝐨𝐭 𝐚𝐟𝐭𝐞𝐫 𝐦𝐚𝐤𝐢𝐧𝐠 𝐭𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭:</b></b></i>"
    elif gateway == "jazzcash":
        text = "<b>𝐀𝐜𝐜𝐨𝐮𝐧𝐭 𝐃𝐞𝐭𝐚𝐢tl𝐬 </b> 📃 \n\n<b>𝗔𝗰𝗰𝗼𝘂𝗻𝘁 :</b> Jazzcash\n<b><b><b><b><b>𝗡𝗮𝗺𝗲 :</b></b></b></b></b> Saira Bibi\n<b><b><b><b><b><b><b><b>𝗡𝘂𝗺𝗯𝗲𝗿 :</b></b></b></b></b></b></b></b> <code>03087886967</code>\n\n⚠️ <i><b><b>𝐏𝐥𝐞𝐚𝐬𝐞 𝐭𝐚𝐤𝐞 𝐚 𝐒𝐜𝐫𝐞𝐞𝐧𝐬𝐡𝐨𝐭 𝐚𝐟𝐭𝐞𝐫 𝐦𝐚𝐤𝐢𝐧𝐠 𝐭𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭:</b></b></i>"
